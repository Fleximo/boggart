...
X2 = pd.DataFrame()
X2['log1p(Score)'] = np.log1p(X1['Score'].values)
X2['Dscore'] = X1['Dscore'].values
X2['log1p(content_dist)'] = np.log1p(X1['content_dist'].values)
...
fig, ax = plt.subplots(figsize=(12, 8))
fig = sm.graphics.plot_fit(mod1, "log1p(Score)", ax=ax)
...
fig, ax = plt.subplots(figsize=(12, 8))
fig = sm.graphics.plot_fit(mod1, "log1p(content_dist)", ax=ax)
